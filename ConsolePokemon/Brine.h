...
class Brine : public Move {
public:

	Brine() : Move("Brine", water, true, 65, 100, 10, 0) {}
	...
	void use_move(BasePokemon* user, BasePokemon* target) override {
		...
		//Double Power if below 50% health
		if (user->get_stats()[1] <= user->get_base_stats()[1] * 0.5) { change_power *= 2; }

		//Solve for damage using gen 1 damage formula
		damage = round(((((((2 * user->get_stats()[0] * crit) / 5) + 2) * change_power * attack / defence) / 50) + 2) * STAB * damage_multiplier * random);
