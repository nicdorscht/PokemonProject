...
class GyroBall : public Move {
public: 
	GyroBall() : Move("Gyro Ball", steel, false, 25, 100, 5, 0) {}
	...
	void use_move(BasePokemon* user, BasePokemon* target) override {
		...
		int modified_power = power * target->get_stats()[6] / user->get_stats()[6];
		...
		damage = round(((((((2 * user->get_stats()[0] * crit) / 5) + 2) * modified_power * attack / defence) / 50) + 2) * STAB * damage_multiplier * random);
